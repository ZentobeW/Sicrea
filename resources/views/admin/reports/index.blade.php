                ...
                @if ($selectedEvent)
                    ...
                    @else
                        <div class="mt-8 space-y-8">
                            <div class="grid gap-4 text-[#5C2518] sm:grid-cols-2 xl:grid-cols-4">
                                <div class="rounded-3xl bg-[#FFF5EF] p-6 shadow-sm">
                                    <p class="text-xs font-semibold uppercase tracking-wide text-[#C06245]">Target Pendapatan</p>
                                    <p class="mt-3 text-2xl font-semibold">{{ $formatCurrency($financialMetrics['target']) }}</p>
                                </div>
                                <div class="rounded-3xl bg-[#FFEDE5] p-6 shadow-sm">
                                    <p class="text-xs font-semibold uppercase tracking-wide text-[#C06245]">Pembayaran Diterima</p>
                                    <p class="mt-3 text-2xl font-semibold">{{ $formatCurrency($financialMetrics['received']) }}</p>
                                    <p class="mt-1 text-xs text-[#A3563F]">Sudah masuk ke kas.</p>
                                </div>
                                <div class="rounded-3xl bg-[#FFF5EF] p-6 shadow-sm">
                                    <p class="text-xs font-semibold uppercase tracking-wide text-[#C06245]">Menunggu Pembayaran</p>
                                    <p class="mt-3 text-2xl font-semibold">{{ $formatCurrency($financialMetrics['pending']) }}</p>
                                </div>
                                <div class="rounded-3xl bg-[#FFEDE5] p-6 shadow-sm">
                                    <p class="text-xs font-semibold uppercase tracking-wide text-[#C06245]">Potensi Refund</p>
                                    <p class="mt-3 text-2xl font-semibold">{{ $formatCurrency($financialMetrics['refunded']) }}</p>
                                </div>
                            </div>
                            ...
                            <div class="rounded-[30px] border border-[#FFE3D2] bg-[#FFF8F4] p-6 shadow-inner">
                                <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                    <div>
                                        <h3 class="text-lg font-semibold text-[#5C2518]">Rincian Status Pembayaran</h3>
                                        <p class="text-sm text-[#A3563F]">Pantau distribusi pembayaran untuk tindak lanjut finance team.</p>
                                    </div>
                                    <a href="{{ route('admin.registrations.index', array_filter(['event_id' => $selectedEventId], fn ($value) => filled($value))) }}" class="inline-flex items-center gap-2 rounded-full bg-white px-4 py-2 text-xs font-semibold text-[#A3563F] shadow">
                                        Kelola Transaksi
                                    </a>
                                ...
                                <div class="mt-5 grid gap-3 md:grid-cols-2">
                                    @forelse ($paymentBreakdown as $breakdown)
                                        <div class="rounded-3xl border border-[#FFE3D2] bg-white/90 p-5 text-sm text-[#5C2518] shadow-sm">
                                            <p class="text-xs font-semibold uppercase tracking-wide text-[#C06245]">{{ $breakdown['status']->label() }}</p>
                                            <p class="mt-2 text-3xl font-semibold">{{ $breakdown['count'] }} peserta</p>
                                            <p class="mt-1 text-xs text-[#A3563F]">{{ $formatCurrency($breakdown['sum']) }}</p>
                                        </div>
                                    @empty
                                        <div class="md:col-span-2 rounded-3xl border border-dashed border-[#FFD1BE] bg-white p-8 text-center text-[#A3563F]">
                    ...
                    @endif
                @else
                    <div class="mt-8 rounded-[30px] border border-dashed border-[#FFD1BE] bg-[#FFF5EF] p-10 text-center text-[#A3563F]">
                        Pilih salah satu event di sebelah kiri untuk melihat ringkasan laporan.
                    </div>
                @endif
            </div>
        </section>
