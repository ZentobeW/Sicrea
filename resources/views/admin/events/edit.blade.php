<x-layouts.app :title="'Edit Event'">
    <div class="max-w-3xl mx-auto">
        <h1 class="text-2xl font-semibold text-slate-800 mb-6">Edit Event: {{ $event->title }}</h1>

        <form method="POST" action="{{ route('admin.events.update', $event) }}" class="bg-white border border-slate-100 rounded-xl shadow-sm p-6 space-y-5">
            @csrf
            @method('PUT')
            <div>
            ...
            <div class="grid md:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-slate-700">Kuota</label>
                    <input type="number" name="capacity" value="{{ old('capacity', $event->capacity) }}" min="1"
                        class="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:border-indigo-500 focus:ring-indigo-500">
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700">Kuota Tersedia</label>
                    <input type="number" name="available_slots" value="{{ old('available_slots', $event->available_slots) }}" min="0"
                        class="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:border-indigo-500 focus:ring-indigo-500">
                </div>
                <div>
            ...
            <div>
                ...
                <textarea name="description" rows="5"
                    class="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('description', $event->description) }}</textarea>
            </div>
            <div class="flex items-center gap-2">
                <input type="hidden" name="publish" value="0">
                <input type="checkbox" name="publish" value="1" id="publish" {{ old('publish', $event->isPublished()) ? 'checked' : '' }}>
                <label for="publish" class="text-sm text-slate-600">Publikasikan</label>
            </div>
            <div class="flex justify-end gap-3">
                <a href="{{ route('admin.events.index') }}" class="text-sm text-slate-500 hover:text-slate-700">Batal</a>
                <button class="inline-flex items-center rounded-md bg-indigo-600 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-700">Perbarui Event</button>
            </div>
        </form>
    </div>
</x-layouts.app>
